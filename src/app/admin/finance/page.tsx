...
export default async function AdminFinancePage() {
    ...
    memberships?.forEach((m) => {
        ...
        monthlyData.forEach((monthItem, index) => {
            ...
            if (startDate >= monthDate && startDate < nextMonth) {
            ...
            <div className="stats-grid" style={{ marginBottom: 'var(--space-8)' }}>
                <div className="stat-card glass-card">
                    ...
                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', margin: 0, marginTop: 'var(--space-2)' }}>
                        From {totalMembers} active memberships
                    </p>
                </div>
                ...
                <div className="stat-card glass-card">
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                            ...
                            <p className="stat-value">{formatCurrency(currentMonthRevenue)}</p>
                        </div>
                        <div style={{
                            width: '48px',
                            height: '48px',
                            borderRadius: 'var(--radius-lg)',
                            background: revenueGrowth >= 0 ? 'var(--color-green)' : 'var(--color-red)',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                        }}>
                    ...
                    <div className={`stat-change ${revenueGrowth >= 0 ? 'positive' : 'negative'}`}>
                        {revenueGrowth >= 0 ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}
                        <span>{revenueGrowth >= 0 ? '+' : ''}{revenueGrowth.toFixed(1)}% vs last month</span>
                    </div>
                </div>
                                ...
                                >
                                    ...
                                    <div style={{ textAlign: 'right' }}>
                                        ...
                                        <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)', margin: 0 }}>
                                            {((loc.total_revenue / totalRevenue) * 100).toFixed(1)}% of total
                                        </p>
                                    </div>
                                </div>
                            ))
                            ...
                            typeArray.map((type, index) => (
                                ...
                                >
                                    ...
                                    <div style={{ textAlign: 'right' }}>
                                        ...
                                        <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)', margin: 0 }}>
                                            {((type.total_revenue / totalRevenue) * 100).toFixed(1)}% of total
                                        </p>
                                    </div>
                                </div>
                            ))
